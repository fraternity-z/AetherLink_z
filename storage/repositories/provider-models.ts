...
export const ProviderModelsRepository = {
  async list(provider?: ProviderId): Promise<ProviderModel[]> {
  ...
  async upsert(provider: ProviderId, modelId: string, label?: string | null, enabled = true): Promise<void> {
    await execute(
  ...
  async remove(provider: ProviderId, modelId: string): Promise<void> {
    await execute(`DELETE FROM provider_models WHERE provider = ? AND model_id = ?`, [provider, modelId]);
  },

  async listOrDefaults(provider: ProviderId): Promise<ProviderModel[]> {
