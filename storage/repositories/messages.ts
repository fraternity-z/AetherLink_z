...
export const MessageRepository = {
  async addMessage(input: {
    ...
    return withRepositoryContext('MessageRepository', 'addMessage', { conversationId: input.conversationId, role: input.role, table: 'messages' }, async () => {
      const id = uuid();
      const createdAt = now();
  ...
  async addMessageWithId(input: {
    ...
    return withRepositoryContext('MessageRepository', 'addMessageWithId', { messageId: input.id, conversationId: input.conversationId, table: 'messages' }, async () => {
      const status = input.status ?? 'sent';
      await execute(
        `INSERT INTO messages (id, conversation_id, role, text, created_at, status, parent_id, extra)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
  ...
  async listMessages(conversationId: string, opts?: { limit?: number; before?: number; after?: number }): Promise<Message[]> {
    return withRepositoryContext('MessageRepository', 'listMessages', { conversationId, options: opts, table: 'messages' }, async () => {
  ...
  async updateMessageText(id: string, text: string): Promise<void> {
    return withRepositoryContext('MessageRepository', 'updateMessageText', { messageId: id, table: 'messages' }, async () => {
  ...
  async updateMessageStatus(id: string, status: 'pending' | 'sent' | 'failed'): Promise<void> {
    return withRepositoryContext('MessageRepository', 'updateMessageStatus', { messageId: id, status, table: 'messages' }, async () => {
  ...
  async deleteMessage(id: string): Promise<void> {
    return withRepositoryContext('MessageRepository', 'deleteMessage', { messageId: id, table: 'messages' }, async () => {
  ...
  async clearConversationMessages(conversationId: string): Promise<void> {
    return withRepositoryContext('MessageRepository', 'clearConversationMessages', { conversationId, table: 'messages' }, async () => {
      // 清空指定对话的所有消息（但保留对话本身）
  ...
  async getAllMessages(): Promise<Message[]> {
    return withRepositoryContext('MessageRepository', 'getAllMessages', { table: 'messages' }, async () => {
      const rows = await queryAll<any>(
        `SELECT id,
                conversation_id as conversationId,
                role,
  ...
  async getMessageCountByRole(): Promise<{ role: Role; count: number }[]> {
    return withRepositoryContext('MessageRepository', 'getMessageCountByRole', { table: 'messages' }, async () => {
      const rows = await queryAll<any>(
  ...
  async getMessageWithThinkingChain(
    ...
    return withRepositoryContext('MessageRepository', 'getMessageWithThinkingChain', { messageId, table: 'messages, thinking_chains' }, async () => {
